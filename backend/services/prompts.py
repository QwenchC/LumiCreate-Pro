...
FRAME_PROMPT_SYSTEM = (
    ...
    "2. CHARACTERS — most critical rule: Include ONLY the characters explicitly listed in the Characters block. "
    "Do NOT add, infer, or import any other character from the description, dialogues, or background knowledge. "
    "If no characters are listed, describe the scene without any named person.\n"
    "3. LOCAL MODEL LIMITATION: Local models produce poor results with many characters. "
    "If only one character is listed, focus the entire prompt on that character. "
    "If multiple characters are listed, keep each description brief and spatially distinct.\n"
    "4. APPEARANCE ISOLATION: Each character's visual tags belong strictly to that character alone. "
    "Never mix, blend, or transfer any visual feature between characters.\n"
    "5. NO STYLE TAGS: Do not include art style, painting style, or rendering style "
    "(no 'anime style', 'watercolor', '3D render', 'photorealistic', 'cartoon', 'manga', 'illustration', etc.).\n"
    "6. CONTENT: Describe environment, lighting, mood, composition, and each listed character's pose and expression.\n"
    "7. OUTPUT: Return ONLY a JSON object with keys \"start_frame_prompt\" and \"end_frame_prompt\". No extra text."
)
...
FRAME_PROMPT_USER_TEMPLATE = (
    ...
    "Write English prompts. Include ONLY the characters listed above.\n"

...
- ✅ **视频生成两个 bug(多智能体排查 + 实盘 ffprobe 验证)**:
  ...
  - **画面放完音频还在响(音画不同步、画面早结束)**: 漫剧 TTS 音频常比生成画面长几十毫秒~数秒。三处重编码都没有按画面封顶 → 输出取最长流(音频) → `-vsync cfr` 下末帧定格到音频放完。**全部按【视频流】时长封顶**(实测验证):
    - 合并两条路径(快路径 concat / 慢路径 filter_complex+BGM)加 `-t video_total`(BGM 淡出/裁剪也对齐到真正片尾)
    - **字幕流水线**(用户实际播放的 `final_video_subbed`):`normalize_video` 产出的 `fixed_cfr` 此前保留了比画面长数秒的音频,烧字幕后整段拖尾;`normalize_video` 与字幕烧录都改为按视频流时长 `-t` 封顶(必须取视频流而非容器时长——容器 = max(画面,音频))
    - 探测失败时安全回退、不封顶,绝不把视频截没

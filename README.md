...
- ✅ **视频生成两个 bug(多智能体排查 + 实盘 ffprobe 验证)**:
  - **ComfyUI 视频重开项目不显示、无法合成**: `GET /videos` 只读 `videos.json`,若该索引缺失/不全(生成时写盘中断或没保存),即使 mp4 已在 `video/` 目录、`scene_assets` 已登记也不显示。改为**自愈**:`videos.json` ∪ `scene_assets(video)` ∪ 盘上实文件,补出的键回写索引(下次直接命中、合成也能读到)

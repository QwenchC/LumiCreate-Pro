...
- **说话人 100% 可控**（v1.5.1）：每条台词的说话人 = 角色表绑定下拉（旁白 + 角色名单），音色按"说话人 → 角色 voice"确定性映射、生成时纯查表不推断；支持 `角色名：台词` 标注剧本确定性解析；「🎭 标注说话人」AI 助手结合上下文为已有台词逐条指派说话人（消解人称代词），再用下拉一键确认
...
- ✅ **③ 朗读双音色按标注切分**(`AudioTab._segmentForDualVoice`)
  - 文本含显式 `角色：台词` 标注 → 确定性按标注切块取音色(说话人 100% 准)；无标注才退回原引号启发式

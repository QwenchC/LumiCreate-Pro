...
- **智能分镜**（v1.6.2）：「🎬 智能分镜」导演式多段续接 —— 把长文案分段、多次调用 LLM 续接编排分镜，避免单次输出上限导致的机械断句与过度简化每个分镜描述；可随时中断
...
- **本镜形象切换**（v1.6.3）：出镜角色若有多个外观，分镜下方出现「🎭 本镜形象」下拉，逐角色选用哪个外观——所选外观的提示词文本注入该镜图片提示词（缺省=常态）
...
- **多外观 / 多形象**（v1.6.3）：同一角色可建多个外观（如「常态」「卧床」「战损」），每个外观一组提示词 + 独立立绘画廊 + 各自主图；其一为「默认/常态」。新建变体外观时「✨ 据常态生成」把常态提示词作为输入派生完整变体提示词，保留角色身份特征（发色/五官/体型）只改服装姿态状态。**在图片/视频生成页可按分镜随时切换某角色用哪个外观**（参见各生成页的「🎭 本镜形象」），无需回角色管理页
- **纯白背景立绘**（v1.6.0）：立绘生成勾选「纯白背景」→ 文本引擎自动把外观改写成纯白底全身参考提示词（强力背景负面词 + 确定性兜底），供 MSR 多图参考视频做角色参考
- **立绘本地上传**（v1.6.2）：「⬆ 上传立绘」从本地选图（任意格式自动转 PNG）直接作为角色立绘，与生成立绘共用同一画廊 / 主图机制（立绘归属当前所选外观）

### 📚 系列连载（v1.6.2）
- **多集共用资源（实时共享池）**：主界面「新建文件夹」选「📚 系列连载」即建一个系列；该系列下各集（项目）**共用角色管理（含立绘）、项目级元素库、项目级音乐库** —— 任一集的改动其余各集立即同步，独立项目不受影响
- **章节文案管理**：系列专属页按章节维护连载文案；新建本系列项目时勾选章节即一键导入到「文案创建」，已用章节标注「第n集：项目名」
- **剧集集数管理**：各集自动「第n集」编号，正序 / 逆序切换；删中间集可选「顺移（连续编号）/ 保留空白集（占位）」
- **主界面开关**：「系列连载项目」开关默认隐藏系列分集（这类项目通常很多），可一键显示；全局搜索不受影响
...
- **本镜形象切换**（v1.6.3）：出镜角色若有多个外观，分镜详情出现「🎭 本镜形象」下拉——所选外观的提示词文本注入该镜视频提示词，MSR 多图参考也严格取该外观的白底立绘（该外观无立绘则把角色从提示词与参考图同时剔除，保持图文同源对齐）
- **跨引擎混合合成**（v1.6.2）：同一分镜的 LTX / MSR / 图片放映 / Seedance 视频分别存到**独立槽**（`<scene>.mp4` / `.msr.mp4` / `.slideshow.mp4` / `.seedance.mp4`），每镜可单独选「合并来源」，不再互相覆盖；合并时统一 scale/pad/setsar + 音频归一化 + 无音轨补静音，异分辨率（如 Seedance 自适应）混排也能正确 xfade/concat
- **master-detail 布局 + 人工审阅**（v1.6.2）：视频生成页改左栏分镜列表 + 右栏当前分镜详情，缓解多视频卡顿；图片/视频生成页各分镜可标记「人工审阅通过」（纯标记，不影响机制）
...
- **试播预览**（v1.4.8，v1.6.2 修复四引擎）：「▶︎ 试播」按钮，合并前不出文件即可在浏览器里按 scene 顺序串播现有素材（三档自动回退 video > image+audio > image-only）；v1.6.2 起按每镜「有效来源」播放，覆盖 LTX / MSR / 图片放映 / Seedance 四引擎槽（此前只认默认 LTX 槽，非默认引擎分镜误显「无素材」）
...
│   ├── src/
│   │   ├── views/              # 页面（HomeView, ProjectView, SettingsView, SeriesView 系列专属页）
│   │   ├── components/
...
├── backend/                    # FastAPI 后端
│   ├── routers/
│   │   ├── projects.py         # 项目 CRUD（场景/图片/音频/视频 存取；含角色立绘上传、系列集数 episode_no）
│   │   ├── series.py           # 系列连载：系列 CRUD + 章节管理/导入 + 剧集集数(shift/blank) + 共享角色池（v1.6.2）
...
│   │   ├── music.py            # ACE-Step v1.5 音乐生成 + 本地音频上传入库（v1.4.2 / v1.6.2）
...
### v1.6.3
本版本给角色加上**多外观 / 多形象**：一个角色可有多个外观，每个外观一组提示词 + 独立立绘，并能在图片/视频生成页**按分镜随时切换**某角色用哪个外观。全程 additive，旧单外观角色自动迁移为「常态」默认外观，不破坏现有角色/立绘/出镜/i2i/MSR/系列共享流程。

- ✅ **角色多外观**（角色管理页）：每角色 `appearances:[{id,name,text,is_default}]`，恰一个默认「常态」；旧单字段 `appearance` 自动迁出默认外观并始终同步。外观标签条 + 当前外观编辑（名 / 设默认 / ✨AI生成 / **✨据常态生成** / 删除）；立绘画廊按当前外观分组，主图 / 白底立绘选取**在每个外观内各自独立**；生成 / 上传立绘归属当前外观。
- ✅ **据常态派生变体**（`POST /text-engine/derive-appearance`）：输入常态外观提示词 + 一句变体说明（如「闭眼盖被躺床上、换睡衣」）→ AI 派生该变体的完整外观提示词，**保留身份特征**（发色 / 五官 / 体型 / 年龄），按说明替换 / 新增服装姿态状态，从而保证各外观形象的相似性。
- ✅ **按分镜切换形象**（图片 / 视频生成页「🎭 本镜形象」）：每分镜每角色一个外观下拉，存 `scene._scene_character_looks={角色名:外观id}`（默认不落键）；该分镜的提示词注入与（视频页 MSR）白底立绘参考都用所选外观；缺省=默认外观。无需回角色管理页即可让「床上」镜用卧床形象、其余镜用常态。
- ✅ **立绘参考按外观对齐**：i2i 参考图 `_describe_ref` 据立绘所属外观注入对应外观文本；MSR 多图参考严格取所选外观的白底立绘，该外观无立绘则把该角色从提示词与参考图同时剔除（图文同源对齐，不回退到别的外观）。
- ✅ **对抗评审（12 confirmed 全修）**：脏 `appearances` 致 500、`_describe_ref` 空名运算符优先级丢描述、derive / AI生成 SSE 误吞错误并清空已写文本、从文案导入缺 `appearances` 致外观编辑器整段消失、设主图跨外观误清主图、图片 / 视频页「本镜形象」切页 / 保存 / KeepAlive 重拉导致丢失（补 saveScenes 触发 + onActivated 保留 + refreshUpstream / onUnmounted flush）、MSR 变体无立绘回退默认致图文不一致、死代码清理。
- ✅ **测试**：后端 **464 / 464 pytest 通过**（v1.6.2 时 455 个），新增多外观 9 例（迁移 / 外观 CRUD / 立绘按外观分组 / 设主作用域 / derive / 脏数据容错）；vite build green。

### v1.6.2
本版本是一个**大型 additive 更新**：新增「系列连载」（多集共用角色/立绘/元素/音乐 + 章节文案 + 集数管理）、视频生成页**跨引擎混合合成**重构，以及智能分镜、立绘/音频本地上传、人工审阅开关等若干功能与 bug 修复。全程遵守用户硬约束「这次更新不能删去旧的机制」，现有 LTX / MSR / Seedance / slideshow / 合并 / 字幕 / BGM 通路 0 破坏。

- ✅ **系列连载（Series）**：主界面「新建文件夹」新增「📚 系列连载」类型 → 创建后端系列实体并进入**系列专属页**
  - **多集共用资源（实时共享池）**：同一系列下各集（项目）**共用角色管理（含立绘）、项目级元素库、项目级音乐库** —— 在任意一集改角色 / 加立绘 / 传元素 / 传曲目，其余各集立即同步（经存储重定向 `_character_base` + 元素 `series:{sid}` scope + 音乐库 `_library_key` 实现；独立项目完全不受影响）
  - **章节文案管理**：系列专属页按章节管理连载文案（增删改）；新建本系列项目时可勾选章节（多选）一键导入到该项目「文案创建」；被某集用过的章节标注「第n集：项目名」避免重复选用
  - **剧集集数管理**：各集自动编号「第n集」，可正序 / 逆序切换；删最后一集直接删，删中间集弹窗二选一 —— **顺移**（后续集号 -1，全系列重排为连续 1..N）或 **保留空白集**（占位、后续集号不变，便于之后重录补回）；空白集可单独删除
  - **主界面「系列连载项目」开关**（默认隐藏）：系列分集通常很多，默认不在主项目页 / 侧栏显示，可一键显示；全局搜索仍可搜到
- ✅ **视频生成页跨引擎混合合成**（每引擎独立存槽，互不覆盖）
  - 同一分镜的 LTX / MSR / 图片放映 / Seedance 视频分别存到**独立槽**（`<scene>.mp4` / `.msr.mp4` / `.slideshow.mp4` / `.seedance.mp4`），每镜可单独选「合并来源」，不再互相覆盖；单镜生成按引擎分派
  - **合并归一化**：跨引擎 / 异分辨率混合时合并 filter_complex 统一 scale / pad / setsar + 音频 aresample / aformat + 无音轨镜补 anullsrc，杜绝 Seedance（自适应分辨率）与 LTX / slideshow 混排时 xfade / concat 失败
  - **master-detail 布局**：视频生成页改为左栏分镜列表 + 右栏当前分镜详情，缓解多视频同时挂载的卡顿
- ✅ **智能分镜（导演式多段续接）**：分镜设计页新增「🎬 智能分镜」，把文案按段切块、多次调用 LLM 续接编排分镜，避免单次输出上限导致的机械断句 / 过度简化每个分镜描述；可中断
- ✅ **本地上传**：
  - **立绘上传**（角色管理页「⬆ 上传立绘」）：本地图片任意格式 → canvas 转码为真 PNG → 复用立绘端点入共享立绘库
  - **音频上传**（音乐库「⬆ 上传音频」）：本地音频（mp3 / m4a / wav / aac / ogg / flac）→ `POST /api/music/upload` 入音乐库（后缀 / 大小校验 + mime 映射 + 系列共享归并）
- ✅ **分镜「人工审阅」开关**：图片 / 视频生成页各分镜可标记「人工审阅通过」（纯标记，不影响任何生成 / 合并机制）
- ✅ **MSR 视频提示词按对白模式差异化**：纯旁白 / 纯对话 / 混合 / 纯朗读四种对白模式下「动作叙述」指导差异化
- ✅ **音频页引擎提示**：选 MSR 多图参考 / 火山引擎 Seedance 引擎时，横幅提示无需在此生成音频
- ✅ **修复（生产反馈）**：
  - **试播无素材**：四槽改造后 MSR / 图片放映 / Seedance 分镜在「▶︎ 试播」里显示「无素材」（试播只认旧单槽 `video`）→ 改为按每镜有效来源播放，覆盖四引擎槽
  - **视频页上游修改不生效**：KeepAlive 保活下视频页只在首次进入读一次上游快照 → 切到角色 / 分镜页改完再切回不生效。改为切回时（`onActivated` + 跨项目 `watch`）刷新只读上游（分镜 / 角色选择 / 外观 / 立绘 / 提示词），**生成中跳过、不动批量进度、epoch 守卫防丢正在键入的提示词** —— 进度保留机制完整保留
- ✅ **质量保障（多智能体 workflow）**：用「并行调查根因 → 实现 → 对抗评审（find → adversarial-verify）」编排，评审查出并修复多个真实缺陷（delete-blank 非正集号腐蚀全系列编号 [CRITICAL]、shift 删集遇旧空白不致密、i2i 立绘参考 404、试播双槽误选引擎、上游刷新丢失在键入的提示词等）
- ✅ **测试**：后端 **455 / 455 pytest 通过**（v1.6.1 时 419 个），新增系列连载 18 例（CRUD / 章节导入 / 共享角色·立绘·元素·音乐 / 集数赋号·shift·blank / i2i 解析）+ 音乐上传 6 例；vite build green
...
- ✅ **无角色背景图容器**（图片生成页）：在首/末帧之外新增第三个「背景图」分镜容器（`frame_type=bg` → `image_bg`），生成不含人物的纯环境图，供 MSR 背景参考
  - **彻底无角色提示词**：背景提示词走专用端点 `POST /text-engine/generate-bg-prompt(-batch)`，LLM 被强约束只描述空场景、绝不出现人物；支持「✦ 全部分镜提示词 / 🖼 全部分镜背景图」批量
- ✅ **MSR 视频提示词生成**（视频生成页）：`POST /text-engine/generate-msr-video-prompt(-batch)` 按「参考图1..N（标签/外观）+ 场景行 + 动作叙述」格式生成中文提示词（与 MSR 工作流自带提示词同构，台词逐字保留 + 镜头运动）。每个 MSR 分镜「✦ 多图参考提示词」单生成 + 工具栏「🎬 全部MSR提示词」批量；角色与参考图同源同序，保证「参考图N 文本」与「LiconMSR 槽位N 图片」严格对齐
...
- ✅ **视频生成两个 bug(多智能体排查 + 实盘 ffprobe 验证)**:
  ...
  - **画面放完音频还没播完(渐进音画错位)**: 逐镜实测**根因** —— AI 生成的分镜视频帧是按**非标准帧率**(常 ~23.7fps)生成的,却被标成 **24fps** 播放,于是每镜画面比其音频**快约 1%**(短几十~一百毫秒)。单镜 <20s 察觉不到,但 80+ 镜拼接后累积成数秒"画面跑在音频前、末尾音频没播完"。之前的 `tpad` 补末帧 / `-t` 封顶只能修末尾,修不了镜内画面整体偏快。**正确修复(逐镜实测验证)**:合并时**逐镜把画面重定时到其音频时长**(`setpts` 按画面本应的速度播放)→ 画面与声音**全程**对齐、无定格、无累积漂移(50 镜实测把 4.4s 累积漂移降到 ~17ms)。**所有合并统一走此路径**(移除了不做重定时的 concat demuxer 快路径,否则无 BGM 合并画面仍偏快)。**片尾**再让画面比音频多撑 0.5s(克隆末帧)→ 最后一句台词务必说完画面才结束,杜绝"视频播完音频还没播完/末句被切"(用户实测确认:画面晚于音频结束时台词完整)
    - **字幕流水线**(`final_video_subbed`):`normalize_video` 与字幕烧录改为按【视频流】时长 `-t` 封顶(容器时长 = max(画面,音频),必须取视频流)
    - 探测失败时安全回退,绝不把视频截没

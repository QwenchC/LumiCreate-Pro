...
- ✅ **视频生成两个 bug(多智能体排查 + 实盘 ffprobe 验证)**:
  ...
  - **画面放完音频还没播完(渐进音画错位)**: 真因不是末尾，而是**逐镜累积**。漫剧每个分镜的 TTS 音频比画面长几十~一百毫秒;含 BGM 的合并走 `concat` filter **分别拼接画面流与音频流**,逐镜的差在拼接时累积成渐进错位 —— 后段台词整体滞后、末尾被裁(之前误用 `-t` 封顶反而把最后一段台词切掉)。**正确修复(逐镜实测验证)**:慢路径合并时**逐镜把画面补到与其音频等长**(`tpad` 克隆末帧)→ 逐镜 v==a → 拼接无累积偏移,画面与声音始终对齐、不丢音频;再按对齐后的总时长 `-t` 安全收尾(并给无限循环 BGM 收口)。快路径(concat demuxer)本就按容器时长对齐,残差 <0.06s
